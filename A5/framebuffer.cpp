...
void FrameBuffer::KeyboardHandle() {
  ...
  switch (key) {
	...
	case FL_Right: {
      ...
      cerr << "pressed right arrow" << endl;  
      break;
    }
	case FL_Up: {
	  ppcc->Tilt(1.0f);
	 
      cerr << "pressed up arrow" << endl;  
      break;
    }
	case FL_Down: {
	  ppcc->Tilt(-1.0f);
	 
      cerr << "pressed down arrow" << endl;  
      break;
    }
    case 'a': {
	  ppcc->Roll(1.0f);
...
void FrameBuffer::SetupEnv(cubeMap *cube){
	V3 direc;
	for(int u=0;u<w;u++){
		for(int v=0;v<h;v++){
			int uv = (h-v-1)*w+u;
			direc=(ppcc->a*(0.5f+float(u))+ppcc->b*(0.5f+float(v))+ppcc->c).UnitVector();
			pix[uv]=cube->cubeGetColor(direc);
			zb[uv]=-1000000000;
		}
	}
...
//currcols[0] = currcols[1] = currcols[2] = V3(1.0f, 0.0f, 0.0f);
...
void FrameBuffer::SetFromImageWithSameEye(FrameBuffer *im1, 
  PPC *ppc1, PPC *ppc) {

  // traverse the destination image
  // for every pixel, look it up in the source image

  for (int v = 0; v < h; v++) {
    for (int u = 0; u < w; u++) {
      V3 projP(.5f+(float)u, .5f+(float)v, 1.0f);
      V3 P = ppc->Unproject(projP);
      V3 projP1;
      if (!ppc1->Project(P, projP1))
        continue;
      int u1 = (int) projP1[0];
      int v1 = (int) projP1[1];
      unsigned int color = im1->GetSafe(u1, v1);
      if (color == 0xFF000000)
        continue;
      unsigned int oldColor = Get(u, v);
      if (oldColor != 0xFF000000) {
        V3 oldColorv;
        oldColorv.SetFromColor(oldColor);
        V3 colorv;
        colorv.SetFromColor(color);
        V3 blendedColorv = (oldColorv + colorv) / 2.0f;
        color = blendedColorv.GetColor();
      }
      Set(u, v, color);
    }
  }
...
float FrameBuffer::DiffAtOverlap(FrameBuffer *im1, 
  PPC *ppc1, PPC *ppc) {
    ...
    for (int v = 0; v < h; v++) {
      for (int u = 0; u < w; u++) {
        ...
        int u1 = (int) projP1[0];
        int v1 = (int) projP1[1];
        unsigned int color1 = im1->GetSafe(u1, v1);
        if (color1 == 0xFF000000)
          continue;
        V3 color1v; color1v.SetFromColor(color1);
        unsigned int color = Get(u, v);
        V3 colorv; colorv.SetFromColor(color);
        V3 diffv = color1v-colorv;
        ret += diffv*diffv;
        errorPixN++;
      }
    }
...
unsigned int FrameBuffer::GetSafe(int u, int v) {
...
// im0 is the image that is moved around to be aligned with "this" image
float FrameBuffer::DiffCorrReproj(FrameBuffer *im0, PPC *ppc0, V3 *corrs0, 
  V3 *corrs1, PPC *ppc1, int corrsN) {

  float ret = 0.0f;
  for (int ci = 0; ci < corrsN; ci++) {
    V3 P = ppc0->Unproject(corrs0[ci]);
    V3 projP;
    if (!ppc1->Project(P, projP))
      return FLT_MAX;
    projP[2] = 1.0f;
    V3 distv = corrs1[ci]-projP;
    ret += distv*distv;
  }
  ret /= corrsN;
  return ret;
...
void FrameBuffer::DrawReflectedMode(TMesh *tm, int tmi, PPC *ppc,cubeMap *cube) {
  ...
  for (int tri = 0; tri < tm->trisN; tri++) {
    ...
    DrawReflectedTriangle(currvs, projcurrvs,currns,center, tmi, ppc, cube);
  }

}
void FrameBuffer::DrawReflectedTriangle(V3 *vs, V3 *pvs, V3 *ns, V3 center,int tmi,PPC *ppc,cubeMap *cube) {
 ...
 // cout<<top<<","<<bottom<<endl;
   ...
   M33 Q = ComputeQ(vs, ppc);
   V3 denDEF = Q[0] + Q[1] + Q[2];
   V3 normalx=V3(ns[0][0],ns[1][0],ns[2][0]);
    V3 normaly=V3(ns[0][1],ns[1][1],ns[2][1]);
	 V3 normalz=V3(ns[0][2],ns[1][2],ns[2][2]);
	float Ax= Q.GetColumn(0)*normalx;
	float Bx= Q.GetColumn(1)*normalx;
	float Cx= Q.GetColumn(2)*normalx;
	float Ay= Q.GetColumn(0)*normaly;
	float By= Q.GetColumn(1)*normaly;
	float Cy= Q.GetColumn(2)*normaly;
	float Az= Q.GetColumn(0)*normalz;
	float Bz= Q.GetColumn(1)*normalz;
	float Cz= Q.GetColumn(2)*normalz;
  for (int v = top; v <= bottom; v++) {
    for (int u = left; u <= right; u++) {
      ...
      float currz = zABC * pixv; // z at current pixel interpolated lin. in s s
      V3 currCol;
	  V3 eyeray=(ppc->a*u+ppc->b*v+ppc->c).UnitVector();
	  V3 unit(1,1,1);
	  V3 n;
	  n[0]=(Ax*u+Bx*v+Cx)/(Q.GetColumn(0)*unit*u+Q.GetColumn(1)*unit*v+Q.GetColumn(2)*unit);
	   n[1]=(Ay*u+By*v+Cy)/(Q.GetColumn(0)*unit*u+Q.GetColumn(1)*unit*v+Q.GetColumn(2)*unit);
	    n[2]=(Az*u+Bz*v+Cz)/(Q.GetColumn(0)*unit*u+Q.GetColumn(1)*unit*v+Q.GetColumn(2)*unit);
	  if(n*eyeray >0){
	     n=V3(0,0,0)-n;
	  }
	  float cos=((V3(0,0,0)-n)*eyeray);
	  V3 direc=(n*cos+eyeray)*2-eyeray;
	  unsigned int color=cube->cubeGetColor(direc);
