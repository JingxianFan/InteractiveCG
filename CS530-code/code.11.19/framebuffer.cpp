...
FrameBuffer* FrameBuffer::Convolve(float **krn, int n) {

  FrameBuffer *ret = new FrameBuffer(0, 0, w, h);

  ret->Set(0xFF000000);

  int n2 = n/2;
  for (int v = n2; v < h-n2; v++) {
    for (int u = n2; u < w-n2; u++) {
      V3 outpix(0.0f, 0.0f, 0.0f);
      float krnsum = 0.0f;
      for (int i = 0; i < n; i++) {
        int currv = v - n2 + i;
        for (int j = 0; j < n; j++) {
          int curru = u - n2 + j;
          V3 inpix;
          inpix.SetFromColor(Get(curru, currv));
          outpix = outpix + inpix*krn[i][j];
          krnsum += krn[i][j];
        }
      }
      outpix[0] = (outpix[0] < 0.0f) ? -outpix[0] : outpix[0];
      outpix[1] = (outpix[1] < 0.0f) ? -outpix[1] : outpix[1];
      outpix[2] = (outpix[2] < 0.0f) ? -outpix[2] : outpix[2];
      outpix[0] = (outpix[0] > 1.0f) ? 1.0f : outpix[0];
      outpix[1] = (outpix[1] > 1.0f) ? 1.0f : outpix[1];
      outpix[2] = (outpix[2] > 1.0f) ? 1.0f : outpix[2];
      ret->Set(u, v, outpix.GetColor());
    }
  }
  return ret;
}
