...
Scene::Scene() {
  ...
  fpfb = new FrameBuffer(u0*2+w, v0, w, h);
  fpfb->label("Fixed Pipeline Framebuffer");
  fpfb->ishw = true;
...
//  tms[2]->Translate(V3(0.0f, +rh2+eps, 0.0f));
  FrameBuffer *texture = new FrameBuffer(0, 0, 4*1024, 4*64);
//  texture->show();
...
//  texture->SetCheckerboard(2, 0xFF000000, 0XFFAAAAFF);
  ...
  tms[3]->SetAsRectangle(corners);
  tms[3]->texture = texture;
...
//  float colorChannel = 114.0f;
  float colorChannel = 154.0f;
//  float colorChannel = 254.0f;
  float xcoord = (colorChannel / 255.0f) * 2.0f;
  xcoord = xcoord - 1.0f;
  xcoord = xcoord * 128.0f;
  cerr << "INFO: x coord is: " << xcoord << endl;
  return;
