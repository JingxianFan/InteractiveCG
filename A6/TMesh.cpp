...
void TMesh::RenderHW(int renderMode) {
  if(renderMode){
	glPolygonMode(GL_FRONT_AND_BACK,GL_LINE);
	if(((float*)cols)[0]!=-1.0f)
		glColor4fv((float*)cols);
	}
  else
  ...
  if((float*)texture){
	...
	unsigned int texturehw;
	glGenTextures(1,&texturehw);
	glBindTexture(GL_TEXTURE_2D,texturehw);
  ...
  if((float*)texture){
	 ...
	 static float texturecoord[]={0,0,1,0,0,1,1,1};
