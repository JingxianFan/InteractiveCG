...
void CGInterface::PerSessionInit() {
  ...
  CGprofile latestBillboardProfile = cgGLGetLatestProfile(CG_GL_FRAGMENT);
  ...
  cout << "Info: Latest FP Profile Supported: " << cgGetProfileString(latestPixelProfile) << endl;
   cout << "Info: Latest BP Profile Supported: " << cgGetProfileString(latestBillboardProfile)<<endl;

  vertexCGprofile = latestVertexProfile;
  pixelCGprofile = latestPixelProfile;
  billboardCGprofile = latestBillboardProfile;
...
bool ShaderOneInterface::PerSessionInit(CGInterface *cgi) {
  ...
  billboardProgram =cgCreateProgramFromFile(cgi->cgContext, CG_SOURCE,"CG/shaderOne.cg",
	  cgi->billboardCGprofile,"BillboardReflectionFragmentShader",NULL);
  if (billboardProgram == NULL)  {
    CGerror Error = cgGetError();
    cerr << "Shader One billboard Program COMPILE ERROR: " << cgGetErrorString(Error) << endl;
    cerr << cgGetLastListing(cgi->cgContext) << endl << endl;
    return false;
  }
	// load programs
	...
	cgGLLoadProgram(billboardProgram);
  ...
  billboardbd00 = cgGetNamedParameter(billboardProgram,"billboard00");
  billboardbd01 = cgGetNamedParameter(billboardProgram,"billboard01");
  billboardbd11 = cgGetNamedParameter(billboardProgram,"billboard11");
  billboardbd10 = cgGetNamedParameter(billboardProgram,"billboard10");
  billboardeye = cgGetNamedParameter(billboardProgram,"cam");
  billboardtexture = cgGetNamedParameter(billboardProgram,"texture");
...
void ShaderOneInterface::PerFrameInit() {
  ...
  V3 V00=scene->tms[4]->verts[0];
   V3 V01=scene->tms[4]->verts[1];
    V3 V10=scene->tms[4]->verts[2];
	 V3 V11=scene->tms[4]->verts[3];

  cgGLSetParameter3f(billboardbd00,V00[0],V00[1],V00[2]);
  cgGLSetParameter3f(billboardbd01,V01[0],V01[1],V01[2]);
  cgGLSetParameter3f(billboardbd11,V11[0],V11[1],V11[2]);
  cgGLSetParameter3f(billboardbd10,V10[0],V10[1],V10[2]);
   V3 cam = scene->ppc->C;
  cgGLSetParameter3f(billboardeye, cam[0], cam[1], cam[2]);
  unsigned int texture=scene->textname;
  cgGLSetTextureParameter(billboardtexture,texture);
...
void ShaderOneInterface::BindPrograms() {
  ...
  cgGLBindProgram(billboardProgram);
}
...
void CGInterface::DisableProfiles() {
  ...
  cgGLDisableProfile(billboardCGprofile);
}
...
void CGInterface::EnableProfiles() {
  ...
  cgGLEnableProfile(billboardCGprofile);
}
