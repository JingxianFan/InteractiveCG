...
void Scene::DBG() {
  ...
  cubefb= new FrameBuffer(10, 250, 100, 100);
  cubefb->LoadTiff("mydbg/env.tiff");
 
  cube= new cubeMap(cubefb);
  canvasfb= new FrameBuffer(20,50,700,700);
  V3 lightPosition;
  float ambientk=0.1;
 
  ppcc= new PPC(70,400,400);
   ppcc->Roll(270.0f);
   ppcc->Pan(35.0f);
  tms[0]->Translate(V3(90,-50,-200.0f));
  canvasfb->ppcc=ppcc;
 
  canvasfb->show();
  canvasfb->SetupEnv(cube);
  canvasfb->DrawReflectedMode(tms[0],0,ppcc,cube);
    for(int i=0;i<50;i++){
		ppcc->C=tms[0]->GetCenter()+V3(0,0,100);
		ppcc->Roll(1.0f);
		 canvasfb->clear();
		 canvasfb->SetupEnv(cube);
		  canvasfb->DrawReflectedMode(tms[0],0,ppcc,cube);
